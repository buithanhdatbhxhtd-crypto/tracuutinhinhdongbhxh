...
# Theo hướng dẫn, để trống api_key để môi trường tự điền lúc chạy
api_key = "" 
...
def call_gemini_with_retry(prompt):
    retries = 5
    delay = 1
    # System Instruction để AI đóng vai chuyên gia BHXH Thuận An
    system_instr = "Bạn là trợ lý ảo thông minh của BHXH cơ sở Thuận An, Lâm Đồng. Hãy trả lời ngắn gọn, chuyên nghiệp, hỗ trợ các đơn vị về chính sách đóng BHXH, BHYT, BHTN."
    ...
    for i in range(retries):
        try:
            ...
            if i == retries - 1:
                return "⚠️ Hệ thống AI đang được bảo trì hoặc bận. Vui lòng thử lại sau giây lát hoặc liên hệ cán bộ chuyên quản qua Zalo/SĐT ở trang chủ để được hỗ trợ trực tiếp."
            time.sleep(delay)
            delay *= 2

# --- CẤU HÌNH TRANG ---
st.set_page_config(
    page_title="BHXH Thuận An - v17.2 Quantum Digital",
    page_icon="🛡️",
    layout="wide",
...
# --- TỔNG LỰC CSS (GIAO DIỆN QUANTUM DIGITAL v17.2 - FIX ALL) ---
st.markdown("""
    <style>
...
def display_pdf_content(file_path):
    try:
        ...
        # Nhúng bằng thẻ object với fallback
        pdf_display = f"""
        <div class="pdf-viewer-box">
            <object data="data:application/pdf;base64,{base64_pdf}" type="application/pdf" width="100%" height="800px">
                <div style="padding: 40px; text-align: center; color: #1e3a8a;">
                    <h4>⚠️ Trình duyệt không hỗ trợ xem trực tiếp</h4>
                    <p>Vui lòng sử dụng nút tải xuống bên dưới để xem văn bản này.</p>
                </div>
            </object>
        </div>
        """
...
with st.sidebar:
    ...
    for t in tabs:
        if st.button(t, use_container_width=True, key=f"tab_{t}"):
            ...
            st.rerun()
    st.divider()
    st.info("Phiên bản v17.2 - Sửa lỗi PDF & AI")

# --- HEADER (LED MARQUEE RGB) ---
marquee_msg = "💎 CHÀO MỪNG ĐẾN VỚI HỆ THỐNG TRA CỨU BHXH THUẬN AN PHIÊN BẢN v17.2 • ĐÃ KHẮC PHỤC LỖI HIỂN THỊ VĂN BẢN • TÍCH HỢP TRÍ TUỆ NHÂN TẠO GEMINI CAO CẤP •"
...
if df is not None:
    ...
    if st.session_state.current_tab == "📊 Tra cứu C12-TS":
        if st.session_state.selected_unit is None:
            ...
            with col_info:
                ...
                OFFICERS = [
                    {"name": "Bà NGUYỄN THỊ NHÀI", "scope": "Xã Đức Lập, Đắk Mil", "phone": "0846.39.29.29", "zalo": "https://zalo.me/0846392929", "class": "card-nhai", "color": "#00d2ff"},
                    {"name": "Ông BÙI THÀNH ĐẠT", "scope": "Xã Đắk Sắk, Đắk Song", "phone": "0986.05.30.06", "zalo": "https://zalo.me/0986053006", "class": "card-dat", "color": "#ffaa00"},
    ...
    # --- TAB 2: AI GEMINI (FIXED) ---
        ...
        st.write("Hệ thống đã được tối ưu. Bạn có thể hỏi về chính sách BHXH ngay.")
        ...
        if prompt := st.chat_input("Hỏi tôi bất cứ điều gì..."):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Đang trích xuất dữ liệu chính sách..."):
                    response = call_gemini_with_retry(f"BHXH Thuận An - Tư vấn: {prompt}")
                    st.markdown(response)
                    st.session_state.messages.append({"role": "assistant", "content": response})

    # --- TAB 3: VĂN BẢN PDF (FIXED ERROR) ---
    elif st.session_state.current_tab == "📂 Văn bản PDF Mới":
        st.markdown("## 📂 THƯ VIỆN VĂN BẢN KỸ THUẬT SỐ")
        pdf_files = get_pdf_files()
        
        if not pdf_files:
            st.warning("Hệ thống chưa tìm thấy file PDF nào. Hãy tải file lên GitHub để kích hoạt thư viện.")
        else:
            c1, c2 = st.columns([1, 2.5])
            with c1:
                st.write("##### Danh sách văn bản:")
                for f in pdf_files:
                    if st.button(f"📄 {f}", use_container_width=True, key=f"pdf_btn_{f}"):
                        st.session_state.active_pdf = f
            with c2:
                if st.session_state.active_pdf:
                    st.success(f"📌 Đang xem: {st.session_state.active_pdf}")
                    p_bytes = display_pdf_content(st.session_state.active_pdf)
                    if p_bytes:
                        st.download_button(
                            label="📥 TẢI VĂN BẢN NÀY VỀ MÁY (KHUYÊN DÙNG)",
                            data=p_bytes,
...
st.markdown("<br><hr><center style='color:#94a3b8; font-size:0.9rem; padding-bottom:60px;'>© 2026 BHXH CƠ SỞ THUẬN AN | Elite Quantum Hub v17.2</center>", unsafe_allow_html=True)