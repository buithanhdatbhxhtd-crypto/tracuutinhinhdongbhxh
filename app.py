...
# --- CẤU HÌNH TRANG ---
st.set_page_config(
    page_title="BHXH Thuận An - v20.4 Crystal Search Hub",
    page_icon="🛡️",
    layout="wide",
...
# --- TỔNG LỰC CSS (GIAO DIỆN CRYSTAL v20.4) ---
st.markdown("""
    <style>
    ...
    * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .stTextInput input {
        ...
        padding: 15px 45px 15px 85px !important; /* Chừa chỗ cho icon kính lúp bên trái */
    ...
    /* Hiệu ứng riêng cho nút TÌM KIẾM chính */
    ...
    .search-main-btn > div > button:hover {
...
# --- DỮ LIỆU CÁN BỘ (BỔ SUNG KEYWORDS ĐỂ MAPPING) ---
OFFICERS = [
    {
        "name": "Bà NGUYỄN THỊ NHÀI", 
        "communes": "Xã Đức Lập, Xã Đắk Mil", 
        "keywords": ["duc lap", "đức lập", "dak mil", "đắk mil", "dakmil", "đắkmil"],
        "phone": "0846.39.29.29", 
    ...
    },
    {
        ...
        "keywords": ["dak sak", "đắk sắk", "daksak", "đắksắk", "dak song", "đắk song", "daksong", "đắksong"],
        "phone": "0986.05.30.06", 
    ...
    },
    {
        ...
        "keywords": ["thuan an", "thuận an", "thuanan", "thuậnan"],
        "phone": "0919.06.11.53", 
...
with st.sidebar:
    ...
    st.divider()
    live_clock_v20_4()
    st.caption("v20.4 Crystal Search | Stable")

# --- HEADER LED ---
marquee_msg = "💎 CHÀO MỪNG ĐẾN VỚI HỆ THỐNG TRA CỨU BHXH THUẬN AN PHIÊN BẢN v20.4 • ĐÃ THÊM NÚT TÌM KIẾM VÀ FIX LỖI CÁN BỘ PHỤ TRÁCH •"
...
if df is not None:
    ...
    if st.session_state.current_tab == "📊 Tra cứu C12-TS":
        if st.session_state.selected_unit is None:
            ...
            with col_news:
                st.markdown("##### 📢 TIN TỨC")
                st.markdown("<div style='background:white; padding:30px; border-radius:30px; border:2px solid #e2e8f0; text-align:center; min-height:350px; display:flex; flex-direction:column; justify-content:center; box-shadow: 0 20px 40px rgba(0,0,0,0.06);'><h4 style='color:#1e3a8a;'>🛡️ AN SINH</h4><p>Đóng BHXH đầy đủ là bảo vệ tương lai bền vững nhất cho NLĐ.</p><hr><small style='color:#f59e0b; font-weight:800;'>BHXH THUẬN AN</small></div>", unsafe_allow_html=True)

            with col_res:
                # Chỉ hiển thị kết quả khi đã có search_query hoặc user gõ (tự động gợi ý)
                final_query = st.session_state.search_query if st.session_state.search_query else user_input
                if final_query:
                    clean_query = unidecode(final_query).lower()
                    ...
                    else: st.error("Không tìm thấy đơn vị phù hợp.")
            ...
            with cr:
                ...
                # HIỂN THỊ CÁN BỘ PHỤ TRÁCH TRỰC TIẾP DỰA TRÊN ĐỊA CHỈ (FIXED)
                st.markdown("##### 👨‍💼 CHUYÊN QUẢN TRỰC TIẾP")
                found_off = False
                for off in OFFICERS:
                    # Kiểm tra xem có từ khóa địa lý nào nằm trong địa chỉ đơn vị không
                    if any(kw in unit_addr for kw in off['keywords']):
                        found_off = True
                        st.markdown(f"""
                        <div style='background: #000; border-radius: 20px; padding: 25px; text-align: center; border: 2px solid {off['color']};'>
                            <small style='color:#39ff14; font-weight:900;'>PHỤ TRÁCH TRỰC TIẾP</small>
                            <h4 style='color:{off['color']}; margin:10px 0; font-size:1.5rem;'>{off['name']}</h4>
                            <p style='color:#aaa; font-size:0.8rem;'>Xã: {off['communes']}</p>
                            <a href='tel:{off['phone'].replace('.','')}' style='color:white; text-decoration:none; font-weight:800; font-size:1.6rem;'>📱 {off['phone']}</a><br>
                            <a href='{off['zalo']}' target='_blank' style='background:#0068ff; color:white; padding:10px 35px; border-radius:50px; text-decoration:none; display:inline-block; margin-top:15px; font-weight:800;'>CHAT ZALO</a>
                        </div>
                        """, unsafe_allow_html=True)
                        break # Tìm thấy là dừng để tránh hiện 2 người
                if not found_off:
                    st.warning("Vui lòng liên hệ bất kỳ cán bộ nào ở trang chủ để được hỗ trợ xã này.")
        ...
        if not pdfs: st.warning("Hãy tải file .pdf lên thư mục GitHub.")
            ...
            with c2:
                if st.session_state.active_pdf:
                    ...
                    embed_pdf_v20_4(st.session_state.active_pdf)
...
st.markdown("<br><hr><center style='color:#94a3b8; font-size:0.9rem; padding-bottom:60px;'>© 2026 BHXH CƠ SỞ THUẬN AN | Elite Quantum Nexus v20.4</center>", unsafe_allow_html=True)
