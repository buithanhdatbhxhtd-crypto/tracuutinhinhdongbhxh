...
# --- CẤU HÌNH TRANG ---
st.set_page_config(
    page_title="BHXH Thuận An - v20.1 Radiant Nexus",
    page_icon="🛡️",
    layout="wide",
    initial_sidebar_state="expanded"
)

# --- CẤU HÌNH AI GEMINI (FIXED VERSION) ---
# Lấy API Key từ Secrets của Streamlit (Settings -> Secrets -> GOOGLE_API_KEY = "...")
...
def get_ai_response(prompt):
    ...
    try:
        # Sử dụng model gemini-1.5-flash (Bản ổn định nhất cho AI Studio hiện tại)
        model = genai.GenerativeModel('gemini-1.5-flash')
        system_instruction = "Bạn là trợ lý ảo cao cấp của cơ quan Bảo hiểm xã hội cơ sở Thuận An, Lâm Đồng. Hãy tư vấn chính sách BHXH chuyên nghiệp, ngắn gọn."
        ...
        if "404" in str(e):
            return "⚠️ **Lỗi Model AI:** Phiên bản Gemini 1.5 Flash chưa được kích hoạt cho Key này hoặc tên model không đúng. Vui lòng kiểm tra lại Google AI Studio."
        return f"⚠️ **Trợ lý AI đang bận:** {str(e)[:150]}... Quý đơn vị vui lòng nhắn tin Zalo cho cán bộ nhé!"
...
# --- TỔNG LỰC CSS (GIAO DIỆN RADIANT v20.1 - SIÊU HIỂN THỊ) ---
st.markdown("""
    <style>
    ...
    * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    /* FIX SIDEBAR CHỮ BỊ TỐI - ÉP MÀU TRẮNG SÁNG */
    [data-testid="stSidebar"] {
        background: linear-gradient(180deg, #0f172a 0%, #1e3a8a 100%) !important;
    }
    
    /* Làm sáng các nhãn trong Sidebar */
    ...
    /* SIÊU Ô TÌM KIẾM GATEWAY - CỰC ĐẠI */
    .gateway-container {
        max-width: 1000px;
...
def live_clock_v2():
    ...
    ">
        <div id="day-text" style="font-size: 0.9rem; font-weight: 600; opacity: 0.8; margin-bottom: 5px;"></div>
        <div id="time-text" style="font-size: 2rem; font-weight: 900; letter-spacing: 2px; text-shadow: 0 0 10px rgba(255,255,255,0.3);"></div>
    </div>
    <script>
        function updateClock() {
            const now = new Date();
            const dateOptions = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
            document.getElementById('day-text').innerText = now.toLocaleDateString('vi-VN', dateOptions).toUpperCase();
        ...
        <div class="pdf-container">
            <object data="data:application/pdf;base64,{base64_pdf}" type="application/pdf" width="100%" height="100%">
                <div style="padding: 100px 40px; text-align: center; background: white; height: 100%;">
                    ...
                    <p style="font-size: 1.2rem; color: #64748b;">Quý đơn vị vui lòng nhấn nút <b>"XEM TRONG TAB MỚI"</b> để xem nội dung.</p>
                </div>
            </object>
        </div>
        """
...
# --- SIDEBAR RADIANT ---
with st.sidebar:
    st.markdown("<h1 style='color:white; text-align:center; font-size: 2rem; font-weight: 900;'>🛡️ QUANTUM</h1>", unsafe_allow_html=True)
    st.divider()
    ...
    st.divider()
    live_clock_v2()
    st.caption("v20.1 Radiant Nexus | Stable")

# --- HEADER LED ---
marquee_msg = "💎 CHÀO MỪNG ĐẾN VỚI HỆ THỐNG TRA CỨU BHXH THUẬN AN PHIÊN BẢN v20.1 • ĐÃ FIX LỖI SIDEBAR VÀ AI GEMINI • TRA CỨU NHANH - CHÍNH XÁC •"
        ...
        if not pdfs: st.warning("Hãy tải file .pdf lên thư mục GitHub.")
            ...
            with c2:
                if st.session_state.active_pdf:
                    ...
                    embed_pdf_v20_1(st.session_state.active_pdf)
...
st.markdown("<br><hr><center style='color:#94a3b8; font-size:0.9rem; padding-bottom:60px;'>© 2026 BHXH CƠ SỞ THUẬN AN | Elite Quantum Nexus v20.1</center>", unsafe_allow_html=True)
